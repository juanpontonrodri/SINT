...
public class FrontEnd {
    ...
    public static void doGetPhase01(PrintWriter out,String comment, String live,String lg) {
...
//in order to avoid switches or ifs in the Front End I have three methods for phase 03 
    ...
    public static void doGetPhase12(PrintWriter out,String country,ArrayList<Album> albums){
        ...
        for (int index = 0; index < albums.size(); index++) {
            if(albums.get(index).getRecord()!=null){
            out.print("<li><a href='?p=holaqueta1&pphase=" + 13 + "&pcountry=" +country + "&paid=" +albums.get(index).getaid()+"'>Album = '"+albums.get(index).getName()+"'</a> --- Year= '"+albums.get(index).getYear()+"' --- Performer='" + albums.get(index).getSinger()+"' --- Review = '" + albums.get(index).getReview()+"' --- Recording = '" + albums.get(index).getRecord()+"'</li>");
            }
            else{
